...
In the workshop I will show you how to deply apps through shinyapps.io. Shinyapps is great, but they don't let you modify data stored on the shinyapps server through the app (**note** you can upload as much static content as you want as long as the whole app is less than 100MB -- but beware of memory usage in the app). 
...
You get to save and access the data "locally". That means that you write data to the server computer, but this code automatically overwrites the csv file with each new user session.

This is an easier way to store data if you are testing an app on your personal computer or if you are hosting your app on your own server (not shinyapps.io). In those cases, you can modify the code to not rewrite the file each time. If you are using shinyapps.io, you'll be able to write the data to the shinyapps.io server, but it will get deleted everytime the remote computer reboots (which is pretty often).
